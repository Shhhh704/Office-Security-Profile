...
export default function WorkplaceDashboard() {
  ...
  const filteredWorkplaces = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter((w) => {
        const zhLine = workplaceDisplayName(w.city, w.name).toLowerCase();
        const i18nLine = workplaceDisplayName(
          w.city ? tr(w.city) : undefined,
          tr(w.name),
        ).toLowerCase();
        return (
    ...
    return result;
  }, [workplaces, searchQuery, sortBy, sortOrder, statusFilter, cityFilter, riskTypeFilter, overdueFilter, tr]);
  ...
  const columns: ColumnsType<WorkplaceSummary> = useMemo(() => [
    {
      title: tr('职场名称'),
      dataIndex: 'name',
      key: 'name',
      fixed: 'left',
      width: 240,
      render: (_: string, record: WorkplaceSummary) => (
        <Link
          to={`/workplace/${record.id}`}
          className="text-sm font-medium text-[#1456F0] hover:underline"
        >
          {workplaceDisplayName(record.city ? tr(record.city) : undefined, tr(record.name))}
        </Link>
      ),
    },
    {
      ...
      onHeaderCell: () => ({ style: { whiteSpace: 'nowrap', width: '1%' } }),
        ...
        return (
          <div className="flex items-center gap-1 w-max">
            {risks.map((risk, index) => (
              ...
              >
                {t(resolveRiskCategoryI18nKey(risk.type))}
              </span>
            ))}
          </div>
        );
    ...
    },
    {
      ...
      render: (safeDays: number | undefined, record: WorkplaceSummary) => (
        record.status === 'green'
          ? (locale === 'en' ? `${safeDays || 0}d` : `${safeDays || 0}天`)
          : <span className="text-gray-400">--</span>
      ),
    },
  ], [tr, t, locale, sortBy, sortOrder]);
      ...
      <div className="w-full flex items-stretch gap-4 mb-5">
        <div className="flex-1 px-5 py-4 bg-ud-body rounded-[12px]">
            <div className="flex flex-col gap-[6px]">
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm text-gray-500">{tr('总职场数')}</div>
              </div>
          ...
          <div className="flex-1 px-5 py-4 bg-ud-body rounded-[12px]">
            <div className="flex flex-col gap-[6px]">
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm text-gray-500">{tr('红灯/黄灯职场')}</div>
              </div>
              <div className="flex items-baseline gap-1">
          ...
          <div className="flex-1 px-5 py-4 bg-ud-body rounded-[12px]">
            <div className="flex flex-col gap-[6px]">
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm text-gray-500">{tr('存在逾期整改的职场')}</div>
              </div>
              <div className="flex items-baseline gap-1">
        ...
        <div className="w-full bg-ud-body p-[20px] rounded-[12px]">
          ...
          <div className="flex items-center gap-4">
            ...
            <Select
              ...
              options={uniqueCities.map((city) => ({ label: tr(city), value: city }))}
            />
            <Select
              value={statusFilter}
              ...
              style={{ width: 'fit-content' }}
              options={[
                { label: tr('红灯'), value: 'red' },
                { label: tr('黄灯'), value: 'yellow' },
                { label: tr('绿灯'), value: 'green' }
              ]}
            />
            <Select
              ...
              allowClear
              style={{ width: 'fit-content' }}
              options={uniqueRiskTypes.map(riskType => ({ label: t(resolveRiskCategoryI18nKey(riskType)), value: riskType }))}
            />
            <Select
              value={overdueFilter}
              onChange={(value) => setOverdueFilter(value as string | undefined)}
              placeholder={tr('整改任务逾期')}
              allowClear
              style={{ width: 'fit-content' }}
              options={[
                { label: tr('是'), value: 'yes' },
                { label: tr('否'), value: 'no' }
              ]}
            />
            {hasFilters && (
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-gray-500">{tr('正在加载职场数据...')}</p>
          </div>
        ) : (
          <>
            {/* Empty State */}
            {filteredWorkplaces.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">{tr('未找到匹配的职场')}</h3>
                <p className="text-gray-500 max-w-sm">{tr('请尝试调整筛选条件或搜索关键词')}</p>
              </div>
            ) : (
              <>
                <Table
                ...
                <div className="flex justify-end mt-[16px]">
                  <Pagination
                    ...
                    showSizeChanger
                    showQuickJumper
                    showTotal={(total) => (locale === 'en' ? `Total ${total}` : `共 ${total} 条`)}
